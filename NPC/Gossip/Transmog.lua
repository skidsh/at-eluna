...
local function DeleteFakeFromDB(itemGUID)
    ...
    CharDBExecute("UPDATE item_instance SET transmog = 0 WHERE guid = "..itemGUID)
end
...
local function OnLogin(event, player)
    ...
    entryMap[playerGUID] = {}
    local result = CharDBQuery("SELECT GUID, transmog FROM item_instance WHERE owner_guid = "..playerGUID.." and transmog > 0")
    if result then
        repeat
